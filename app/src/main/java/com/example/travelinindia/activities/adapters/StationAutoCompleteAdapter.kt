...
class StationAutoCompleteAdapter(
    context: Context,
    private val stationList: List<StationNameAndCode>
) : ArrayAdapter<String>(context, R.layout.preferences_list_layout), Filterable {

    private val stationFilter = StationFilter()
    private val stationListFull: List<StationNameAndCode> = ArrayList(stationList)
    ...
    override fun getItem(position: Int): String? {
        return ("${stationList[position].stationName}, ${stationList[position].stationCode}")
    }
    ...
    override fun getView(position: Int, convertView: View?, parent: ViewGroup): View {
        ...
        val text = "${station?.stationName}, ${station?.stationCode}"
    ...
    inner class StationFilter : Filter() {
        override fun performFiltering(constraint: CharSequence?): FilterResults {
            ...
            val filteredList: MutableList<StationNameAndCode> = ArrayList()
            ...
            if (constraint.isNullOrEmpty()) {
                ...
                for (station in stationListFull) {
                    if (station.stationName.contains(filterPattern) || station.stationCode.contains(filterPattern)) {
